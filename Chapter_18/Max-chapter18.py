...
prompt = ChatPromptTemplate.from_messages([
    ...
    when you use the write_file tool, you always save the path where the saved
    file is stored using the download_path tool."""),
...
def YouTube_scraper(url):
    with st.spinner(f"Scraping subtitles from the YouTube Video {url}"):
        try:

            loader =  YoutubeLoader.from_youtube_url(url, add_video_info=False)
            return {"content": loader.load()}
        except Exception as e:
            return {"content": str(e)}

# Define the web scraping tool
YouTube_subs_scraping_tool = Tool(
    name="youtube_scraper",
    func=YouTube_scraper,
    description="A tool to scrape subtitles from a youtube video"
)

MyYouTubeSearchTool = YouTubeSearchTool()
...
with TemporaryDirectory() as tempdir:
                ...
                ftools = FileManagementToolkit(
                root_dir=str(tempdir),
                selected_tools=["read_file", "write_file", "list_directory"],
                ).get_tools()

# Load the tools we are going to provide to Agent
read_tool, write_tool, list_tool = ftools

def savepathtosession():
    st.session_state.temp_folder = write_tool.root_dir
    print(f"Path saved to session: {write_tool.root_dir}")
    return

download_path_tool = Tool(
    name="download_path",
    func=savepathtosession,
    description="A tool save the directory path where the files are stored"
)
...
with st.sidebar:
    ...
    if ("temp_folder" in st.session_state) & (st.session_state.temp_folder != []):
        for root, dirs, files in os.walk(st.session_state.temp_folder):
            for file in files:
                file_path = os.path.join(root, file)
                with open(file_path, "rb") as f:
                    print(f"Adding {file} to the sidebar")
                    file_bytes = f.read()
                    if (file!="HEAD") & (file!="main") :
                ...
                st.session_state.download_files.append(file)
